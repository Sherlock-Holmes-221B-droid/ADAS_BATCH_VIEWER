    ...
    with all_ph:
        ...
        if table.empty:
            ...
            if not USE_AGGRID_LOCAL:
                def color_rows(row):
                    status = row.get('status', row.get('state', 'Error'))
                    return ['background-color: #eaf7ee' if status == 'Pass'
                            else ('background-color: #fdeceb' if status == 'Fail'
                                  else 'background-color: #f7f7f7')] * len(row)
                st.dataframe(table.style.apply(color_rows, axis=1), use_container_width=True)
