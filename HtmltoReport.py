...
def safe_dataframe(df: pd.DataFrame, height: int = 420):
    """
    Cloud-safe DataFrame rendering (no AgGrid)
    """
...
def export_ppt_tables_only(df: pd.DataFrame) -> bytes:
    ...
    tf = slide.shapes.add_textbox(
        Inches(0.8), Inches(1.2), Inches(4.5), Inches(3)
    ).text_frame
    ...
    ) if total else "No data available"

    # ---------- Scenario Tables ----------
    table_cols = [
        "scenario_name", "state", "status",
        "ego_speed", "overlap",
        "impact_speed", "stop_distance",
        "t_aeb", "aeb_activation",
        "result_code"
    ]
    ...
    while start < len(df):
        chunk = df.iloc[start:start + rows_per_slide][table_cols]
        slide = prs.slides.add_slide(prs.slide_layouts[5])
        slide.shapes.title.text = f"Scenarios {start+1} – {start+len(chunk)}"

        rows, cols = len(chunk) + 1, len(table_cols)
    ...
    with c1:
        if st.button("Export PowerPoint (tables only)"):
            if total == 0:
                ...
                ppt = export_ppt_tables_only(view)
                st.download_button(
                    "Download PPTX",
                    ppt,
                    "ADAS_Batch_Report.pptx",
                    "application/vnd.openxmlformats-officedocument.presentationml.presentation"
                )
    
    with c2:
        if st.button("Export Excel"):
            xls = export_excel(view)
            st.download_button(
                "Download Excel",
                xls,
                "ADAS_Batch_Report.xlsx",
